...
// Filter berdasarkan prodi, hari, dan kelas (avoid escaping until query time)
$prodi_filter = isset($_GET['prodi']) ? $_GET['prodi'] : '';
$hari_filter = isset($_GET['hari']) ? $_GET['hari'] : '';
...
$hari_options = mysqli_query($conn, "SELECT DISTINCT hari FROM jadwal_matkul ORDER BY FIELD(hari, 'SENIN', 'SELASA', 'RABU', 'KAMIS', \"JUM'AT\", 'SABTU')");
    ...
    <header>
        ...
        <div class="user-dropdown">
            ...
            <div class="dropdown-content">
                ...
                <?php if ($userType === 'Admin' || $userType === 'PJ Kelas'): ?>
                    <a href="PilihKelas.php?hari=<?= urlencode($hari_filter) ?>&jam=">📅 Pilih Kelas</a>
                <?php endif; ?>
            ...
            <form method="GET" class="d-inline">
                ...
                <select name="hari" id="hari" class="form-select d-inline w-auto" onchange="this.form.submit()">
                    ...
                    <?php while ($hari_row = mysqli_fetch_assoc($hari_options)): ?>
                        <option value="<?= htmlspecialchars($hari_row['hari']) ?>" <?= $hari_filter === $hari_row['hari'] ? 'selected' : '' ?>>
        ...
        <table class="table table-bordered table-striped">
            ...
            <tbody>
                ...
                while ($row = $result->fetch_assoc()) {
                    ...
                    if ($userType === 'Admin') {
                        echo "<a href='EditJadwalKelas.php?id={$row['id']}' class='btn btn-warning btn-sm'>Edit</a>
                              <a href='HapusJadwalKelas.php?id={$row['id']}' class='btn btn-danger btn-sm' onclick='return confirm(\"Yakin ingin menghapus?\")'>Hapus</a>";
                    } else {
                        echo "-"; // Display a placeholder for PJ Kelas, Mahasiswa, and Dosen
                    }
                    echo "</td>
                    </tr>";
                    $no++;
