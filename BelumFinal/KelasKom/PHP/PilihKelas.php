...
function tampilkan_ruangan($ruangan_semua, $isi_kelas, $room_status, $temp_changes, $filter_callback, $hapus_prefix = '') {
    ...
    foreach (array_unique($ruangan_semua) as $ruangan) {
        ...
        if (isset($temp_changes[$ruangan])) {
            $status = 'temporary';
            $class = 'temporary';
            $info = $temp_changes[$ruangan]['kelas'] . ' - ' . $temp_changes[$ruangan]['mata_kuliah'] . ' (' . $temp_changes[$ruangan]['prodi'] . ')';
            $extra_info = '<span class="used-by">Digunakan sementara oleh ' . htmlspecialchars($temp_changes[$ruangan]['kelas']) . '</span>';
            ...
            } elseif ($userType === 'PJ Kelas' && !isset($room_status[$ruangan])) {
