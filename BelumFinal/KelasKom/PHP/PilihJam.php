...
// Verify if hari exists in the database
$query_validate = mysqli_query($conn, "SELECT COUNT(*) as count FROM jadwal_matkul WHERE hari = '$hari'");
...
// Ambil daftar jam berdasarkan hari dari database
$query_jam = mysqli_query($conn, "SELECT DISTINCT jam FROM jadwal_matkul WHERE hari = '$hari' ORDER BY jam");
?>
...
<head>
  ...
  <title>KelasKom - Pilih Jam</title>
  <link rel="stylesheet" href="../CSS/PilihJam.css">
</head>
<body>
  <header>
  ...
  <main>
    <h3>Pilih Jam untuk Hari: <?= htmlspecialchars($hari) ?></h3>
    <div class="day-grid">
      <?php while ($row = mysqli_fetch_assoc($query_jam)): ?>
        <a href="PilihKelas.php?hari=<?= urlencode($hari) ?>&jam=<?= urlencode($row['jam']) ?>">
          <div class="day-card"><?= htmlspecialchars($row['jam']) ?></div>
        </a>
      <?php endwhile; ?>
    </div>
  </main>
